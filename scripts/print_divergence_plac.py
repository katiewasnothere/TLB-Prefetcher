...
for bench in sim_list:
	...
	if "gap" in suite:
		output= open('output/'+suite+'/divergence_plac/'+bench_name+'.txt', 'r')
	elif "cloudsuite" in suite:
		output= open('output/'+suite+'/divergence_plac/mix'+str(i)+'.txt', 'r')
		i+= 1
	else:
		output= open('output/spec06/divergence_plac/'+bench_name+'.txt', 'r')		
